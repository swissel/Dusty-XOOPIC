...
# IMPORTANT!!!! Must Define which Tcl/Tk versions are being used.
#               Uncomment the appropriate define.
# TCL_TK_DEFINES = -DTcl74_Tk40
...
# Note that for each architecture the FFLAGS (Fortran compiling
# options) depend on the flags and libraries needed when using
# the Fortran procedure "getarg". To determine these flags for
# your system, do a "man getarg".

LIBFOBJS =	xgfinterface.o

LIBCOBJS = 	xgcrosshair.o xginitc.o xginitvar.o xgpaint2d.o xgpaint3d.o 		xgpaintvec.o xgpixmap.o xgprint.o xgsetup.o xgcommands.o 		xgreadrit.o xgrescale.o xgprintgif.o xgmovie.o xginitf.o
...
all:: libXGF250$(FILE_EXT).a

libXGF250$(FILE_EXT).a:  $(LIBFOBJS) $(EXTRALIBRARYDEPS)
	$(RM) $@
	$(AR) $@  $(LIBFOBJS)
	$(RANLIB) $@
	$(_NULLCMD_)

all:: libXGC250$(FILE_EXT).a

libXGC250$(FILE_EXT).a:  $(LIBCOBJS) $(EXTRALIBRARYDEPS)
	$(RM) $@
	$(AR) $@  $(LIBCOBJS)
	$(RANLIB) $@
	$(_NULLCMD_)

install:: libXGC250$(FILE_EXT).a
	@if [ -d $(DESTDIR)$(XGPATH) ]; then set +x; \
	else (set -x; $(MKDIRHIER) $(DESTDIR)$(XGPATH)); fi
	$(INSTALL) $(INSTALLFLAGS) $(INSTLIBFLAGS) libXGC250$(FILE_EXT).a $(DESTDIR)$(XGPATH)
	$(RANLIB) $(RANLIBINSTFLAGS) $(DESTDIR)$(XGPATH)/libXGC250$(FILE_EXT).a

install:: libXGF250$(FILE_EXT).a
	@if [ -d $(DESTDIR)$(XGPATH) ]; then set +x; \
	else (set -x; $(MKDIRHIER) $(DESTDIR)$(XGPATH)); fi
	$(INSTALL) $(INSTALLFLAGS) $(INSTLIBFLAGS) libXGF250$(FILE_EXT).a $(DESTDIR)$(XGPATH)
	$(RANLIB) $(RANLIBINSTFLAGS) $(DESTDIR)$(XGPATH)/libXGF250$(FILE_EXT).a

install:: xgrafix.h
	@if [ -d $(DESTDIR)/local/include ]; then set +x; \
	else (set -x; $(MKDIRHIER) $(DESTDIR)/local/include); fi
	$(INSTALL) $(INSTALLFLAGS) $(INSTDATFLAGS) xgrafix.h $(DESTDIR)/local/include/xgrafix.h
