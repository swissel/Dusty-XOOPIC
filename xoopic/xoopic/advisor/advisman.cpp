...
AdvisorManager::AdvisorManager(const char* fileName)
{
    ...
    ListOfAllParameterGroups.add(new ostring("SpeciesVar"));
...
#endif
      for (spIter.restart(); !spIter.Done(); spIter++)
          if (spIter.current()->get_speciesName()==ostring("dust")) {
...
ostring AdvisorManager::readSpeciesVar(ifstream& fin)//dust
{
	char buffer[512];
