...
Grid::Grid(int _J, int _K, Vector2** g,int geometry, BOOL _PeriodicFlagX1, 
			  BOOL _PeriodicFlagX2)
{
	J = _J;
	K = _K;
			...
			for (j=0; j<=J; j++)
			{
				...
				for (k=0; k<=K; k++)
				{
					...
					if (axis()) // apply radial half cell vol corrections
					  if (k == 0) halfCellVolumes[j][k] /= radial0;
					  else if (k==K) halfCellVolumes[j][k] /= radialK;
					rm2 = rp2;
				}
			}
		break;
...
void Grid::init_cellVolume(void)
{
	...
	switch(geometryFlag)
		...
		case ZXGEOM: 
			for (j=0; j<J; j++)
				for (k=0; k<K; k++)
					cellVolume_array[j][k] = (X[j][k+1].e2() - X[j][k].e2())*
						(X[j+1][k].e1() - X[j][k].e1());
			break;
		case ZRGEOM:
		default: 
			for (j=0; j<J; j++)
				for (k=0; k<K; k++)
					cellVolume_array[j][k] = M_PI*(X[j][k+1].e2()*X[j][k+1].e2()
						- X[j][k].e2()*X[j][k].e2())*(X[j+1][k].e1()	- X[j][k].e1());
			break;
		}
}
