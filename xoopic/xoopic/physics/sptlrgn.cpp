      ...
      if (s < nSpecies)
        {
          ...
          for (pgIter.restart(); !pgIter.Done(); pgIter++)
          {
              if (p->get_speciesPtr()->get_dust()&&pgIter()->get_dust()) {
          ...
          if (pgIter.Done())			//	no space; add new group
            {
             ParticleGroup*  pg;
                if (p->get_speciesPtr()->get_dust()) //variable charge dust
                {  
                    pg =new ParticleGroupVAR(maxN, p->get_speciesPtr(),p->get_np2c(),p->isVariableWeight());
                }		
              else  if(getInfiniteBFlag() && getElectrostaticFlag())
...
void SpatialRegion::addParticleVarList(ParticleVarList& particleList)//unnecessary for dust
{
    // empty Particle list into available ParticleGroups
    oopicListIter<ParticleVar> pIter(particleList);
    for (pIter.restart(); !pIter.Done(); pIter++)
    {
...
int SpatialRegion::Restore_2_00(FILE* DMPFile)
  ...
  while (!(speciesID==-1)){
    ...
    // Making a new particleGroup
      ...
      ParticleGroup* pg;  
      if (species->get_dust()) //variable charge dust
      {
          pg =new ParticleGroupVAR(maxN, species, np2c0, vary_np2c);
      }		
    else  if(getInfiniteBFlag() && getElectrostaticFlag())
...
//  Species* species=0;
  ...
  while (!(speciesID==-1)){
    ...
    {
//      if (speciesIter.current()->getID() == speciesID)
//          if (speciesIter.current().get_name==ostring("dust")) {
//              SpeciesVar* species  = dynamic_cast<SpeciesVar*> speciesIter.current();
//
 //         } else {
          Species* species=speciesIter.current();
