...
Scalar dCrossSection::sigmaDelastic(Scalar energy,Scalar q)// dust elastic from chutov
{
    ...
    return (q*q*logcoulomb/(16*M_PI*EPS0*EPS0*energy*energy));
}
Scalar dCrossSection::sigmaDdepositionI(Scalar energy,Scalar q)
{
    return (M_PI*r*r*(1+q/(4*M_PI*energy*r*EPS0)));
}
Scalar dCrossSection::sigmaDdepositionE(Scalar energy,Scalar q)
{
    return (M_PI*r*r*(1-q/(4*M_PI*energy*r*EPS0)));
}
                   ...
                   discardDumpFileNGDDataFlag)
{
    ...
    {
        if (sIter.current()->get_dust())
        {
            dust=TRUE;
            r=sIter.current()->get_r();//dust radius-static
            d=sIter.current()->getID();
            mdust=sIter.current()->get_m();
...
{//bulk of method moved to collideto be evaluated every time
    ...
    if (gasDensity > 0) {
        ...
        if (dust) {
            dust_average_charge =new Scalar*[J+1];
            for (int i=0; i<=J; i++) {
                dust_average_charge[i]=new Scalar[K+1];
            }
            dust_average_chargesquared= new Scalar*[J+1];
            for (int i=0; i<=J; i++) {
                dust_average_chargesquared[i]=new Scalar[K+1];
            }
            dustdensity = new Scalar* [J+1];
            for (int i=0; i<=J; i++) {
                dustdensity[i]=new Scalar[K+1];
            }
            dustchargedensity = new Scalar* [J+1];
            for (int i=0; i<=J; i++) {
                dustchargedensity[i]=new Scalar[K+1];
            }
        }
...
MCCPackage::~MCCPackage()
{
  ...
  delete gasDistribution;
    if (dust) {
        for (int i=0; i<=J; i++) {
            delete [] dustchargedensity[i];
            delete[]dustdensity[i];
            delete []dust_average_charge[i];
            delete []dust_average_chargesquared[i];
        }
        delete [] dustchargedensity;
        delete[]dustdensity;
        delete []dust_average_charge;
        delete []dust_average_chargesquared;
    }
...
//original start of method
  ...
  while (nCollisions--){
    ...
    try{
      if ( ptrNGD->getNGD(iNGD, jNGD) > 0.0 ||get_dust_density()[iNGD][jNGD] > 0.0) {
    ...
    if ((collisionModel == electron)||(collisionModel == test)) 
    {
        ...
        else//dust
        {
            ...
            {
                if((eCX[i]->get_type()!=CrossSection::DUST_DEPOSITION_ELECTRON)&&((eCX[i]->get_type()!=CrossSection::DUST_ELASTIC_ELECTRON)))
                {
                    try
                    {
                        sumSigma += ecxFactor*eCX[i]->sigma(energy)*(ptrNGD->getNGD(iNGD,jNGD))/maxNGD;
                ...
                else
                {
                    try
                    {
                        dCrossSection* deCX=dynamic_cast<dCrossSection*> (eCX[i]);
                        if ((eCX[i]->get_type())==CrossSection::DUST_DEPOSITION_ELECTRON )
                        {
                            sumSigma += ecxFactor*deCX->sigma(energy,get_dust_average_charge_per_particle(iNGD,jNGD))*(get_dust_density()[iNGD][jNGD])/(get_rho_dust_MAX()*get_dust_density_MAX());
                        }
                        else if((eCX[i]->get_type())==CrossSection::DUST_ELASTIC_ELECTRON)
                            sumSigma += ecxFactor*deCX->sigma(energy,sqrt(get_dust_average_chargesquared_per_particle()[iNGD][jNGD]))*(get_dust_density()[iNGD][jNGD])/(get_rho_dust_MAX()*get_dust_density_MAX());
                    }
                    catch(Oops& oops2){
                        oops2.prepend("MCCPackage::collide: Error: \n");//OK
                        throw oops2;
    ...
    else if (collisionModel == ion) 
    {
      ...
      // loop over all possible collisions with this gas
        ...
        else//dust
        {
            for (int i=0; i<niCX; i++)
            {
                if((iCX[i]->get_type()!=CrossSection::DUST_ELASTIC_ION)&&(iCX[i]->get_type()!=CrossSection::DUST_DEPOSITION_ION))
                {
                    try
                    {
                    sumSigma += icxFactor*iCX[i]->sigma(energy)*(ptrNGD->getNGD(iNGD,jNGD))/maxNGD;
                ...
                else
                {
                    try
                    {
                        dCrossSection* diCX=dynamic_cast<dCrossSection*> (iCX[i]);
                        if ((iCX[i]->get_type())==CrossSection::DUST_DEPOSITION_ION )
                        {
                              sumSigma += icxFactor*diCX->sigma(energy,get_dust_average_charge_per_particle(iNGD,jNGD))*(get_dust_density()[iNGD][jNGD])/(get_rho_dust_MIN()*get_dust_density_MAX());
                        }
                        else if((iCX[i]->get_type())==CrossSection::DUST_ELASTIC_ION)
                            sumSigma += icxFactor*diCX->sigma(energy,sqrt(get_dust_average_chargesquared_per_particle()[iNGD][jNGD]))*(get_dust_density()[iNGD][jNGD])/(get_rho_dust_MAX()*get_dust_density_MAX());
                    }
                    catch(Oops& oops2){
                        oops2.prepend("MCCPackage::collide: Error: \n");//OK
                        throw oops2;
  ...
  for (Scalar energy=0; energy<MAX_ENERGY; energy+=0.5) 
    {
      ...
      for (int i=0; i<neCX; i++) 
      {
    if((eCX[i]->get_type()!=CrossSection::DUST_ELASTIC_ELECTRON)&&(eCX[i]->get_type()!=CrossSection::DUST_DEPOSITION_ELECTRON))
...
Scalar MCCPackage::sumdSigmaVe()
{
    ...
    for (Scalar energy=0; energy<MAX_ENERGY; energy+=0.5) 
    {
        ...
        { 
            ...
            {
                dCrossSection* deCX=dynamic_cast<dCrossSection*> (eCX[i]);
                temp += deCX->sigma(energy,get_rho_dust_MAX());
            }
        }
        maxSigmaVe = MAX(maxSigmaVe, sqrt(energy)*temp);
    }
...
Scalar MCCPackage::sumdSigmaVi()
{
    ...
    for (Scalar energy=0; energy<MAX_ENERGY; energy+=0.5) 
    {
        ...
        //	 energy, sqrt(energy)*temp, maxSigmaVi);
            dCrossSection* diCX=dynamic_cast<dCrossSection*> (iCX[i]);
        if((iCX[i]->get_type())==CrossSection::DUST_ELASTIC_ION)
            temp += diCX->sigma(energy,get_rho_dust_MAX());
            else if ((iCX[i]->get_type())==CrossSection::DUST_DEPOSITION_ION)
                temp += diCX->sigma(energy,get_rho_dust_MIN());
        }
        maxSigmaVi = MAX(maxSigmaVi, sqrt(energy)*temp);
    }
    maxSigmaVi *= icxFactor/sqrt(iEnergyScale);
...
throw(Oops){
  switch(cx.get_type()) 
	 ...
	 case CrossSection::CHARGE_EXCHANGE:
		chargeExchange(cx);
		break;
    case CrossSection::DUST_DEPOSITION_ION:
             dustDepositionIon(cx);
        break;
    case CrossSection::DUST_ELASTIC_ION:
        dustElasticIon(cx);
        break;
    case CrossSection::DUST_DEPOSITION_ELECTRON:
        dustDepositionElectron(cx);
        break;
    case CrossSection::DUST_ELASTIC_ELECTRON:
        dustElasticElectron(cx);
        break;             
	 default:
...
void MCCPackage::dustDepositionElectron(CrossSection& cx)
{
//    pg->remove(index);
}

void MCCPackage::dustDepositionIon(CrossSection& cx)
{
...
{ 	// This is the original, nonrelativistic algorithm
   // if (relativisticMCC == 0) {
        u /= (v+1E-30); // normalize
        newVelocity(energy, v, u, TRUE,FALSE,TRUE); // scatter electron
//	}
    
 /* 	// This is the new relativistic algorithm, leave for later
    else {
...
{ 	// This is the original, nonrelativistic algorithm
   // if (relativisticMCC == 0) {
        u /= (v+1E-30); // normalize
        newVelocity(energy, v, u, TRUE,TRUE); // scatter ion
	//}
...
void MCCPackage::newVelocity(Scalar energy, Scalar v, Vector3& u, bool eFlag,BOOL iondustflag,BOOL electrondustflag)
{
	Scalar	up1, up2, up3;
	Scalar	coschi;
	Scalar	sinchi;
...
Scalar** MCCPackage::get_dust_average_charge_per_particle()
{
    ...
    Scalar q,jx,kx;
    Vector2 *x;
    oopicListIter<ParticleGroup> pgIter(*(region->getParticleGroupListArray()[d]));
    for (pgIter.restart(); !pgIter.Done(); pgIter++)
    {
    ParticleGroupVAR* pgd= dynamic_cast<ParticleGroupVAR*>( pgIter.current());
        x = pgd->get_x(); 
    Scalar** _charge=new Scalar*[J+1];
    for (int i=0; i<=J; i++) {
        _charge[i]=new Scalar[K+1];
    }
    Scalar** _dustnumber=new Scalar*[J+1];
    for (int i=0; i<=J; i++) {
        _dustnumber[i]=new Scalar[K+1];
    }
        for(int i=0; i<=pgd->get_n(); i++)
        {
            jx = x[i].e1();
            kx = x[i].e2();
            ...
            q = pgd->get_q(i);
            _charge[j][k] += (1-jx)*(1-kx)*q;
            _dustnumber[j][k] += (1-jx)*(1-kx);
            _charge[j][k+1] += (1-jx)*(kx)*q;
            _dustnumber[j][k+1] += (1-jx)*(kx);
            _charge[j+1][k] += (jx)*(1-kx)*q;
            _dustnumber[j+1][k] += (jx)*(1-kx);
            _charge[j+1][k+1] += (jx)*(kx)*q;
            _dustnumber[j+1][k+1] += (jx)*(kx);
            for (int l=0; l<=J; l++) 
            {
                for (int y=0; y<=K; y++) 
                {
                    dust_average_charge[l][y]=_charge[l][y]/_dustnumber[l][y];
                }
            }
        }
    }
    return dust_average_charge;
}
Scalar** MCCPackage::get_dust_average_chargesquared_per_particle()
{
    int j,k;
    Scalar q,jx,kx;
    Vector2 *x;
    ...
    ParticleGroupVAR* pgd= dynamic_cast<ParticleGroupVAR*>( pgIter.current());
        x = pgd->get_x();
    Scalar** _charge2=new Scalar*[J+1];
    for (int i=0; i<=J; i++) {
        _charge2[i]=new Scalar[K+1];
           }
    Scalar** _dustnumber=new Scalar*[J+1];
    for (int i=0; i<=J; i++) {
        _dustnumber[i]=new Scalar[K+1];
    ...
    for(int i=0; i<=pgd->get_n(); i++)
    {
        ...
        jx -= j; kx -= k;  //get the fractional part only
        q = pgd->get_q(i);
        _charge2[j][k] += (1-jx)*(1-kx)*q*q;
        _dustnumber[j][k] += (1-jx)*(1-kx);
        _charge2[j][k+1] += (1-jx)*(kx)*q*q;
        _dustnumber[j][k+1] += (1-jx)*(kx);
        _charge2[j+1][k] += (jx)*(1-kx)*q*q;
        _dustnumber[j+1][k] += (jx)*(1-kx);
        _charge2[j+1][k+1] += (jx)*(kx)*q*q;
        _dustnumber[j+1][k+1] += (jx)*(kx);
        for (int x=0; x<J+1; x++) 
        {
            for (int y=0; y<K+1; y++) 
            {
                dust_average_chargesquared[x][y]=_charge2[x][y]/_dustnumber[x][y];
            }
        }
    }
    }
...
Scalar** MCCPackage::get_dust_density()
{
    int i,j,k;
    Scalar q,jx,kx;
    Vector2 *x;
    ...
    ParticleGroupVAR* pgd= dynamic_cast<ParticleGroupVAR*>( pgIter.current()); 
            x = pgd->get_x();
    for(i=0; i<=pgd->get_n(); i++) {
        jx = x[i].e1();
        kx = x[i].e2();
        j = (int)jx; k=(int)kx;  //get the integer part
        jx -= j; kx -= k;  //get the fractional part only
        dustdensity[j][k] += (1-jx)*(1-kx);
        dustdensity[j][k+1] += (1-jx)*(kx);
        dustdensity[j+1][k] += (jx)*(1-kx);
        dustdensity[j+1][k+1] += (jx)*(kx);
    }
        }
    return dustdensity;
...
void ArMCC::addCrossSections()
{
 ...
 // electron model
  ...
  if(dust)
        {
    eCX[3]= new  dCrossSection(r,  CrossSection::DUST_DEPOSITION_ELECTRON);
    eCX[4]= new  dCrossSection(r, CrossSection::DUST_ELASTIC_ELECTRON);
        }
...
/*dust not used
Scalar ArMCC::sigmaDelastic(Scalar energy,Scalar q)// dust elastic from chutov
{
    Scalar logcoulomb=10;//coulomb logarithm estimation
    return (q*q*logcoulomb/(8*TWOPI*8.854e-12*8.854e-12*energy*energy));
}
Scalar ArMCC::sigmaDdepositionI(Scalar energy,Scalar q)
{
    return (TWOPI/2*r*r*(1+q/(2*TWOPI*energy*r*8.854e-12)));
}
Scalar ArMCC::sigmaDdepositionE(Scalar energy,Scalar q)
{
    return (TWOPI/2*r*r*(1-q/(2*TWOPI*energy*r*8.854e-12)));
}*/
