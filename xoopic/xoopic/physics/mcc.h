...
class CrossSection
{
  ...
  public:
    ...
    enum Type { ELASTIC, EXCITATION, IONIZATION, ION_ELASTIC,
                CHARGE_EXCHANGE, TEST_TO_BOLTZMANN,DUST_DEPOSITION_ION,DUST_ELASTIC_ION,DUST_DEPOSITION_ELECTRON,DUST_ELASTIC_ELECTRON } type;
    ...
    virtual Scalar sigma(Scalar energy, Scalar q)
    {
        switch (get_type()) {
            case CrossSection::DUST_DEPOSITION_ION:
                return sigmaDelastic(energy, q);
                break;
            case CrossSection::DUST_ELASTIC_ION:
                return sigmaDelastic(energy, q);
                break;
            case CrossSection::DUST_DEPOSITION_ELECTRON:
                return sigmaDdepositionE(energy, q);
                break;
            case CrossSection::DUST_ELASTIC_ELECTRON:
                return sigmaDdepositionI(energy, q);
            default:
                printf("unrecognized collision type!\n");
                return 0;
        }  
    }//real function
    virtual Scalar sigma(Scalar energy){return 0;}//dummy function to implement base class
};
...
class MCCPackage : public BaseMCPackage
{
 private:
  ...
  Scalar sumSigmaVi();    // returns MAX(sigma*vi)
    ...
    Scalar sumdSigmaVe();//same for dust
 ...
 protected:
  ...
  Vector3 newMomentum(const Vector3 U_initial, const Scalar& energy,
    ...
    Scalar** dust_average_charge;
    Scalar** dust_average_chargesquared;
 public:
  ...
  virtual ~MCCPackage();
  virtual void init(SpeciesList& _sList, Scalar dt);
...
class ArMCC : public MCCPackage
{
  ...
  /*   Scalar sigmaDelastic(Scalar energy, Scalar q);//dust not used
    Scalar sigmaDdepositionI(Scalar energy, Scalar q);
     Scalar sigmaDdepositionE(Scalar energy, Scalar q);*/
 public:
